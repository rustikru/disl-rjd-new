...
<title>Дислокация вагонов — РЖД</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .stat-pill .lbl {
    font-size: 9px; color: rgba(255,255,255,.5); letter-spacing: .1em; text-transform: uppercase;
  }
...
function renderSidebar() {
  ...
  STATIONS.forEach(s => {
    ...
    const loaded = s.wagons.filter(w => w.ld).length;
    const empty = s.count - loaded;
...
function selectStation(code) {
  ...
  document.getElementById('dp-loaded').textContent = s.wagons.filter(w=>w.ld).length;
  document.getElementById('dp-empty').textContent  = s.wagons.filter(w=>!w.ld).length;
...
function renderWagonList(s, search) {
  const wagons = getFilteredWagons(s).filter(w =>
    !search || w.n.includes(search)
  );
  const list = document.getElementById('wagon-list');
...
function updateStats() {
  const total = STATIONS.reduce((a, s) => a + countByFilter(s), 0);
  const stationsWithWagons = STATIONS.filter(s => countByFilter(s) > 0).length;
...
document.getElementById('wagon-search').addEventListener('input', e => {
