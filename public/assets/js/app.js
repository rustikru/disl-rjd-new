...
// ================================================================
// WAGON_TABS — конфиг вкладок «Подход / Отправление / Погрузка»
//
// Поля:
//   ctx          — идентификатор контекста; должен совпадать с ключом
//                  в DETAIL_CONTEXTS (detail-contexts.js)
//   summaryUrl   — API сводной таблицы (/api/xxx/summary)
//   detailUrl    — API детализации (/api/xxx/detail)
//   filtersUrl   — API значений для фильтров (/api/xxx/filters)
//
//   metricsId    — id контейнера KPI-карточек в app.php;
//                  убери поле — карточки не появятся
//   metricsLabel — подпись главной KPI-карточки («Всего…»)
//   buildMetrics(data) — [опц.] переопределяет набор карточек;
//                  получает ответ API, возвращает [{label,value,accent?}]
//
//   csvFilename  — префикс имени скачиваемого файла («подход» →
//                  подход_2026-06-11.csv); убери поле — кнопки CSV нет
//
//   sumTableId   — id <table> сводной таблицы в app.php
//   sumSubId     — id подписи под заголовком сводной таблицы
//   sumSubLabel  — префикс текста подписи: «Всего: 142 ваг.»
//   detTableId   — id <table> детализации
//   detSubId     — id подписи под заголовком детализации
//   detPanelId   — id inner-панели детализации (нужен чтобы знать,
//                  открыта ли она при нажатии «Применить»)
//
//   loadedKey    — имя глобальной переменной-флага: сводная уже
//                  загружалась (чтобы не дублировать запрос)
//   loadedDetKey — то же для детализации
//
//   groupCols    — колонки группировки строк сводной таблицы:
//                  [{key, label}], key = поле БД, label = заголовок;
//                  порядок = иерархия: первый — дорога, последний — станция
//
//   getParams()  — возвращает параметры фильтров для API-запросов
//                  (читает текущие значения <select> / <input>)
//   fillFilters(data) — заполняет <select> фильтров из ответа filtersUrl
//   resetFilters()    — сбрасывает <select> фильтров в пустое значение
// ================================================================
