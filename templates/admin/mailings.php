...
<head>
  ...
  <style>
    ...
    .details-box{width:min(760px,96vw);max-height:90vh;overflow:auto;background:var(--surface);border-radius:14px;box-shadow:0 24px 70px rgba(27,23,38,.28)}
    .details-head{position:sticky;top:0;z-index:1;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:17px 20px;border-bottom:1px solid var(--border);background:var(--surface)}
    .details-title{font-size:16px;font-weight:700}
    ...
    .details-section{margin-bottom:20px}.details-section:last-child{margin-bottom:0}
    .details-section h3{margin:0 0 10px;font-size:13px;color:var(--text-2)}
    .details-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 24px}
    .details-item{padding:9px 0;border-bottom:1px solid var(--border)}
    ...
    .details-value{font-size:13px;color:var(--text-1);white-space:pre-wrap;overflow-wrap:anywhere}
    .details-wide{grid-column:1/-1}
    .details-list{margin:0;padding-left:18px;font-size:13px}.details-list li+li{margin-top:5px}
    ...
    @media(max-width:900px){.main-content{padding:18px}.admin-head{align-items:flex-start}}
...
<div class="app-body">
  ...
  <main class="main-content">
    ...
    <section class="panel">
      ...
      <?php else: ?>
        <div class="table-wrap"><table class="data-table">
          <thead><tr><th>Рассылка</th><th>Пользователь</th><th>Организация</th><th>Отчёт</th><th>Расписание</th><th>Получатели</th><th>Статус</th><th>Следующий запуск</th><th>Последний результат</th></tr></thead>
          <tbody>
          <?php foreach ($mailings as $mailing): ?>
            ...
            <tr class="open-details" tabindex="0" role="button" aria-label="Открыть настройки рассылки <?= htmlspecialchars((string) $mailing['name']) ?>" data-mailing-id="<?= (int) $mailing['id'] ?>">
              ...
              <td><?= htmlspecialchars((string) ($mailing['organization_short_name'] ?: ($mailing['organization_name'] ?? '—'))) ?></td>
              <td><?= htmlspecialchars($reportLabels[$mailing['report_code']] ?? (string) $mailing['report_code']) ?><span class="muted"><?= htmlspecialchars((string) $mailing['report_view']) ?> · XLSX</span></td>
    ...
    <section class="panel" id="run-history">
      ...
      <?php else: ?>
        <div class="table-wrap"><table class="data-table">
          <thead><tr><th>Запуск</th><th>Рассылка</th><th>Пользователь</th><th>Организация</th><th>Статус</th><th>Дата отчёта</th><th>Файл / ошибка</th></tr></thead>
          <tbody>
          <?php foreach ($runs as $run): ?>
            ...
            <tr>
              ...
              <td><strong><?= htmlspecialchars((string) $run['mailing_name']) ?></strong><span class="muted"><?= htmlspecialchars($reportLabels[$run['report_code']] ?? (string) $run['report_code']) ?></span></td>
              <td><?= htmlspecialchars((string) ($run['display_name'] ?: $run['username'])) ?><span class="muted"><?= htmlspecialchars((string) $run['username']) ?></span></td>
              <td><?= htmlspecialchars((string) ($run['organization_short_name'] ?: ($run['organization_name'] ?? '—'))) ?></td>
              ...
              <td><?php if (!empty($run['error_message'])): ?><div class="error-text"><?= htmlspecialchars((string) $run['error_message']) ?></div><?php else: ?><?= htmlspecialchars((string) ($run['file_name'] ?? '—')) ?><?php endif; ?></td>
            </tr>
          <?php endforeach; ?>
          </tbody>
        </table></div>
...
<?php foreach ($mailings as $mailing): ?>
  <?php
    $report = $reports[$mailing['report_code']] ?? [];
    $filterLabels = [];
    foreach (($report['filters'] ?? []) as $filter) {
        $filterLabels[(string) ($filter['name'] ?? '')] = (string) ($filter['label'] ?? ($filter['name'] ?? ''));
    }
    $filters = [];
    foreach ($filterLabels as $key => $label) {
        $filters[$key] = $mailing['filters'][$key] ?? null;
    }
    foreach ((array) ($mailing['filters'] ?? []) as $key => $value) {
        if (!array_key_exists((string) $key, $filters)) {
            $filters[(string) $key] = $value;
        }
    }
    $viewLabel = strtoupper((string) $mailing['report_view']) === 'SUMMARY' ? 'Сводный' : 'Подробный';
  ?>
  <div class="details-wrap" id="mailing-details-<?= (int) $mailing['id'] ?>" role="dialog" aria-modal="true" aria-labelledby="mailing-title-<?= (int) $mailing['id'] ?>">
    <div class="details-box">
      <div class="details-head">
        <div class="details-title" id="mailing-title-<?= (int) $mailing['id'] ?>"><?= htmlspecialchars((string) $mailing['name']) ?></div>
      ...
      <div class="details-body">
        <section class="details-section">
          ...
          <div class="details-grid">
            ...
            <div class="details-item"><span class="details-label">Организация</span><div class="details-value"><?= htmlspecialchars((string) ($mailing['organization_name'] ?? '—')) ?></div></div>
            <div class="details-item"><span class="details-label">Отчёт</span><div class="details-value"><?= htmlspecialchars($reportLabels[$mailing['report_code']] ?? (string) $mailing['report_code']) ?></div></div>
            <div class="details-item"><span class="details-label">Вид и формат</span><div class="details-value"><?= htmlspecialchars($viewLabel) ?> · XLSX</div></div>
            <div class="details-item"><span class="details-label">Расписание</span><div class="details-value"><?= htmlspecialchars((string) $mailing['schedule_label']) ?></div></div>
            <div class="details-item"><span class="details-label">Состояние</span><div class="details-value"><?= (int) $mailing['is_active'] === 1 ? 'Активна' : 'Отключена' ?></div></div>
            ...
            <div class="details-item"><span class="details-label">Следующий запуск</span><div class="details-value"><?= htmlspecialchars($dateText($mailing['next_run_at'] ?? null)) ?></div></div>
        ...
        <section class="details-section">
          ...
          <?php if (!$filters): ?>
            <div class="details-value">Фильтры не заданы</div>
          <?php else: ?>
            <div class="details-grid">
              <?php foreach ($filters as $key => $value): ?>
                <?php
                  ...
                  if ($valueText === '') $valueText = 'Не задан';
                ?>
                <div class="details-item"><span class="details-label"><?= htmlspecialchars($filterLabels[(string) $key] ?? (string) $key) ?></span><div class="details-value"><?= htmlspecialchars($valueText) ?></div></div>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>
        </section>
