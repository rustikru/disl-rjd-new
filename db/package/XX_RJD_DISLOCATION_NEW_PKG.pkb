create or replace package body xx_rjd_dislocation_new_pkg as
   ...
   begin
      ...
      if p_date is null then
         ...
         l_sql := 'SELECT COUNT(*) FROM xx_dislocation_rjd'
                  ...
                  || ' ) AND '
                  || l_kpi_where;
         dbms_output.put_line(' 2_l_sql=' || l_sql);
         execute immediate l_sql
           into l_value
            using p_date,p_date;
      end if;
   ...
   begin
      ...
      p_file_data := null;
        /*
        apps.xx_get_dir_list (p_directory);
   ...
   begin
      ...
      using (
         select trim(p_esr_code) as esr_code,
           ...
           from dual
      ) src on ( s.esr_code = src.esr_code )
      when matched then update
      set s.station_name = src.station_name,
